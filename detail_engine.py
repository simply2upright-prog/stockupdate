# detail_engine.py — v3: Boden/Top-Zielkurse, NaN-Guards, auto_adjust=False
...
def _detect_patterns_and_targets(df: pd.DataFrame) -> tuple[list, dict]:
    """
    Mustererkennung + Zielkurs-Berechnung für Boden- und Topformationen.
    Gibt (patterns_list, targets_dict) zurück.
    """
    patterns = []
    targets  = {}          # z.B. {"Doppelboden_Partial": 123.4, "Doppelboden_Full": 130.1}
    ...
    try:
        ...
        p_max_i = list(df.index[idx_max])[-6:]   # Zeitstempel
        ...
        if len(p_min) >= 2:
            ...
            if b1 > 0 and abs(b1-b2)/b1 < 0.03:
                ...
                neckline  = df['High'].loc[p_min_i[-2]:p_min_i[-1]].max() if p_min_i[-2] != p_min_i[-1] else price
                depth     = neckline - min(b1, b2)
                targets["Doppelboden_Nackenlinie"]  = round(neckline, 2)
                targets["Doppelboden_Ziel_50pct"]   = round(neckline + depth * 0.5, 2)
                targets["Doppelboden_Ziel_100pct"]  = round(neckline + depth, 2)
        ...
        # Selling Climax / Kapitulation
        rvol_v = df['RVOL'].iloc[-1] if 'RVOL' in df.columns else 0
        if pd.notna(rvol_v) and df['Close'].iloc[-1] < df['Open'].iloc[-1] and float(rvol_v) > 2.0:
            patterns.append("Selling Climax (Potenzielle Wende)")
            # Einstieg: bei Stabilisierung über Intraday-Hoch
            targets["Selling_Climax_Einstieg"] = round(float(df['High'].iloc[-1]), 2)

        # V-Boden (starker Rebound nach Absturz)
        if len(df) >= 10:
        ...
        if len(p_max) >= 2:
            ...
            if t1 > 0 and abs(t1-t2)/t1 < 0.03:
                ...
                try:
                    neckline = df['Low'].loc[p_max_i[-2]:p_max_i[-1]].min() if p_max_i[-2] != p_max_i[-1] else price
                except Exception:
                    neckline = price * 0.95
                depth    = max(t1,t2) - neckline
                targets["Doppeltop_Nackenlinie"]  = round(neckline, 2)
                targets["Doppeltop_Ziel_50pct"]   = round(neckline - depth * 0.5, 2)
                targets["Doppeltop_Ziel_100pct"]  = round(neckline - depth, 2)

        # SKS-Formation (klassisch)
        if len(p_max) >= 3:
        ...
        # Buying Climax (Erschöpfungsrally)
        if pd.notna(rvol_v) and df['Close'].iloc[-1] > df['Open'].iloc[-1] and float(rvol_v) > 2.5:
            patterns.append("Buying Climax (Erschöpfungsrally)")
            targets["Buying_Climax_SL"] = round(float(df['Low'].iloc[-1]), 2)

        # ── ALLGEMEINE ZIELE ───────────────────────────────────────
        # Fibonacci Retracement vom 52W-Swing
        df_1y  = df.tail(252)
        ...
        if len(df) >= 2:
            ...
            if gap > 0.015:
                label = "Power Gap Up" if df['Volume'].iloc[-1] > vol_a * 1.5 else "Gap Up"
                patterns.append(f"{label} (+{round(gap*100,1)}%)")
            elif gap < -0.015:
                label = "Power Gap Down" if df['Volume'].iloc[-1] > vol_a * 1.5 else "Gap Down"
                patterns.append(f"{label} ({round(gap*100,1)}%)")
