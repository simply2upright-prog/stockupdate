# app.py — Globaler Strategie Scanner mit Gruppen-Filter & Währungs-Support
...
st.caption(f"🌍 Datenbank: {get_ticker_count()} einzigartige Ticker aus {len(get_all_groups())} Märkten & Sektoren")
...
with tab1:
    ...
    col_grp, col_min = st.columns([3, 1])
    all_groups    = ["🌍 Alle Gruppen"] + get_all_groups()
    selected_group = col_grp.selectbox("📂 Markt / Index / Sektor:", all_groups, key="scan_group")
    min_score      = col_min.selectbox("Min. Score:", [1, 2, 3], index=0)

    run_scan = st.button("🔥 SCAN STARTEN", use_container_width=False)

    if run_scan:
        tickers = (
            get_all_tickers()
            if selected_group == "🌍 Alle Gruppen"
            else get_tickers_by_group(selected_group)
        )
        raw_data = []
...
with tab2:
    ...
    selected_ticker = st.selectbox(
        ...
        options=all_tickers,
        help="Ticker aus der globalen Datenbank wählen.",
        key="detail_ticker",
    )
    ...
    if selected_ticker:
        ...
        if data and "df" in data:
            ...
            st.error(
                f"❌ Keine Daten für **{selected_ticker}** gefunden.  \n"
                f"Mögliche Ursachen: Symbol falsch, Delisting, kein Internetzugang zum API."
            )

# ══════════════════════════════════════════════════════════════════════════════
# TAB 3 — REVERSAL SCANNER
# ══════════════════════════════════════════════════════════════════════════════
with tab3:
    st.header("🛡️ Reversal-Scanner & Bodenbildung")

    mode = st.radio("Modus:", ["🔍 Markt scannen", "🎯 Einzelne Aktie prüfen"], horizontal=True)

    if mode == "🔍 Markt scannen":
        grp_opts  = ["🌍 Alle Gruppen"] + get_all_groups()
        grp_t3    = st.selectbox("Gruppe:", grp_opts, key="reversal_group")
            ...
            if data and "df" in data:
                ...
                with col_l:
                    fig_g = go.Figure(go.Indicator(
                        mode="gauge+number", value=prob_score,
                    ...
                    fig_g.update_layout(paper_bgcolor="rgba(0,0,0,0)", font={"color": "white"}, height=400)
...
with tab4:
    ...
    st.info(
        "Der tägliche Report wird automatisch via **GitHub Actions** gesendet (täglich 06:24 UTC).  \n"
        "Hier kannst du ihn manuell auslösen."
    )
